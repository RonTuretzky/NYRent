...
const FLOWS: Flow[] = [
  ...
  },
  {
    ...
    n: 2,
    icon: PlugsIcon,
    title: "Browse market and connect",
    blurb: "Everything is readable without a wallet; connect only to act.",
    steps: [
      "Open Market and pick one — payout curve, lifecycle, capacity and solvency bars.",
    ...
    alt: "Browsing the market list and Market #0 detail while disconnected, then connecting a wallet through the RainbowKit modal until the account chip shows in the header.",
    width: 900,
    height: 506,
  },
  {
  ...
  },
  {
    ...
    blurb: "Pay a fixed-rate premium, mint non-transferable RENT 1:1 with your max claim.",
    steps: [
  ...
  },
  {
    ...
    steps: [
      ...
      "The market locks at its ratio — 61% for the 2026-09-17 issue. One shot; the first successful qualifying settlement wins.",
    ],
    media: "settle.gif",
  ...
  },
  {
    ...
    blurb: "After settlement, burn RENT for maxClaim × ratio — redemption can never be paused.",
    steps: [
...
const RULES: { pool?: boolean; body: ReactNode }[] = [
  ...
  },
  {
    ...
    body: (
      <>
        A market settles against an observation whose <em>signed email time</em>{" "}
  ...
  },
  {
    ...
    body: (
      <>
        Per-market escrow: every claim unit is backed 1:1 by its own market's
        escrow (<Code>sold ≤ escrow</Code>), claims draw only from that
        escrow, and the creator's per-market pause never blocks settle or
    ...
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <ul className="font-parkBody text-sm text-text-standard mt-4 space-y-1">
          {(
            [
              ...
              ["Settle Market #2", LIVE_DEMO_TXS.settle],
      ...
      <Card>
        ...
        <p className="font-parkBody text-sm text-text-standard">
          ...
          capacity from the caller as that market's escrow. Every unit of
          protection sold is backed 1:1 by that escrow alone: accounting is
          strictly per market, so one creator's claims can never touch
          another's money.
        </p>
        <ul className="font-parkBody text-sm text-surface-grey-2 mt-3 space-y-1.5 list-disc pl-5">
          <li>
            Creator rights (and nothing more): pause their own market's sales,
      ...
      <Card>
        ...
        <p className="font-parkBody text-sm text-text-standard">
          ...
          works both legs — UNDERWRITING new standard market when premiums
      ...
      <Card>
        ...
        <p className="font-parkBody text-sm text-surface-grey-2 mb-2">
          ...
          kept here purely as the historical record: its market 0 settled at
          0.61 with the real 2026-09-17 newsletter (emailId{" "}
          <span className="font-mono text-xs">
            {truncateHex(SETTLEMENT_EMAIL_ID, 6)}
          </span>
          ), and its market 1 was wound down — free capital withdrawn while
        ...
        <p className="font-parkBody text-sm text-surface-grey-2 mt-3 mb-2">
          Market 0's full life — three permanent transactions:
        </p>
